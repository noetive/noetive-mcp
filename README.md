...
```sh
git config core.hooksPath .githooks   # runs `claude plugin validate` before each commit
...
Never hand-edit `packaging/claude-plugin`, `packaging/kiro-power`, `.claude-plugin/` or `.mcp.json` — they are generated by `make emit` from `tools/manifest.yaml`, and CI fails when they differ.
